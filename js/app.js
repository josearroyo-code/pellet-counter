/* ══════════════════════════════════════════
   Pellet Counter v7.2
   Motor: Claude Sonnet Vision AI
   Mejoras acumuladas de todas las versiones
   ══════════════════════════════════════════ */
...
/* ── perfiles optimizados por tamaño ── */
...
window.saveApiKey = function() {
  ...
  if (!key.startsWith('sk-ant-')) {
    showApiStatus('✗ Clave inválida — debe empezar por sk-ant-', '#f97316'); return;
  }
...
window.saveProductDesc = function() {
  localStorage.setItem('productDesc', qs('#productDesc').value);
};
...
const DEFAULT_PRODUCTS = {
  pellets: { name:'Pellets electrodo', icon:'⬤', isDefault:true }
};
...
window.renderProductSelector = function() {
  ...
  el.innerHTML   = Object.entries(products).map(([id,p]) =>
    `<button onclick="selectProduct('${id}')" style="flex:1;min-width:70px;justify-content:center;flex-direction:column;gap:2px;padding:8px 4px;font-size:11px;${id===active?'background:var(--blue-dim);border-color:var(--blue);color:var(--blue)':''}">
      <span style="font-size:15px">${p.icon}</span><span>${p.name}</span>
    </button>`
  ).join('') +
  `<button onclick="showAddProduct()" style="flex:1;min-width:55px;justify-content:center;flex-direction:column;gap:2px;padding:8px 4px;font-size:11px;">
    <span style="font-size:15px">➕</span><span>Nuevo</span>
  </button>`;
};
...
window.loadProfile = function(key) {
  ...
  /* descripción según producto y tamaño */
  let desc = '';
...
function highlightProfile(key) {
  qsa('.prof-btn').forEach(b => {
    ...
    b.style.background   = isActive ? 'var(--blue-dim)' : '';
    b.style.borderColor  = isActive ? 'var(--blue)' : '';
...
Sustituye los valores 0 por los conteos reales. confidence debe ser "alta", "media" o "baja". notes es string o null.`;
  ...
  try {
    ...
    if (!text.startsWith('{')) {
      const m = text.match(/\{[\s\S]*?"small"[\s\S]*?\}/);
      if (m) text = m[0];
      else throw new Error('La IA no devolvió formato JSON. Pulsa "Analizar de nuevo".');
    }
    ...
    counts = {
      ...
      c8:    result.medium || 0,
      c12:   result.large  || 0,
      total: result.total  || (result.small||0)+(result.medium||0)+(result.large||0)
    };
    if (sizeMode === 'single') {
      counts.c4 = 0; counts.c12 = 0;
      counts.c8 = counts.total;
    }
    ...
    if (albaranQty > 0) {
      ...
      else if (diff < 0)   albEl.innerHTML = `<span style="color:#f97316">⚠️ FALTAN ${Math.abs(diff)} uds respecto al albarán (${albaranQty})</span>`;
      else                 albEl.innerHTML = `<span style="color:#f59e0b">ℹ️ SOBRAN ${diff} uds respecto al albarán (${albaranQty})</span>`;
    }

    /* status con confianza */
    ...
    setStatus('statusCount', `✓ ${counts.total} detectados · Confianza: ${result.confidence}${notesText}`, null);
...
function buildOdooText() {
  ...
  const mode  = qs('#sizeMode')?.value || 'single';
  const sz    = qs('#singleSize')?.value || '8';
  ...
  if (diff !== null) {
    lines.push(diff===0 ? '✓ COINCIDE con albarán' : diff<0 ? `⚠️ FALTAN ${Math.abs(diff)} uds` : `ℹ️ SOBRAN ${diff} uds`);
  }
...
window.buildOdoo = function() {
  const el = qs('#odooBlock');
  if (el) el.textContent = buildOdooText();
};

window.copyOdoo = function() {
  navigator.clipboard.writeText(buildOdooText())
...
window.calcGrav = function() {
  ...
  const total = parseFloat(qs('#gravTotal').value);
...
function buildOdooGrav(size, qty) {
  ...
  const now=new Date(), seq=Math.floor(Math.random()*900)+100;
  const pad=n=>String(n).padStart(3,'0');
...
window.updateOdooGrav = function() {
  const size=qs('#gravSize').value;
  const qty=parseInt(qs('#gravQty').textContent.replace(/\D/g,''))||0;
  if (qty>0) buildOdooGrav(size,qty);
};
window.copyOdooGrav = function() {
  navigator.clipboard.writeText(qs('#odooBlockGrav').textContent)
...
function updateHistoryBadge() {
  const h = loadHistory();
  const b = qs('#historyBadge');
  if (b) b.textContent = h.length > 0 ? h.length : '';
}
...
window.renderHistory = function() {
  ...
  const h  = loadHistory();
  if (h.length === 0) {
    el.innerHTML = '<p style="color:var(--muted);font-size:13px;text-align:center;padding:24px">Sin análisis aún</p>';
    return;
  }
  el.innerHTML = h.map((e,i) => {
    const diffText = e.albaran
      ? (counts.total===e.albaran ? ' · ✓ OK' : ` · Δ${e.total-e.albaran}`) : '';
    const confColor = e.confidence==='alta'?'#3ecf8e':e.confidence==='media'?'#f59e0b':'#f97316';
    return `
    <div style="background:var(--surface);border:0.5px solid var(--border);border-radius:var(--radius);padding:12px;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
        <span style="font-size:15px;font-weight:700;color:var(--text)">${e.total} uds${diffText}</span>
        <span style="font-size:10px;padding:2px 8px;border-radius:10px;background:${confColor}22;color:${confColor};font-weight:600">${e.confidence||'—'}</span>
      </div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:4px">${e.date}</div>
      <div style="font-size:11px;color:var(--hint);margin-bottom:6px">${(e.product||'').slice(0,55)}…</div>
      ${e.notes?`<div style="font-size:11px;color:var(--muted);font-style:italic;margin-bottom:6px">${e.notes}</div>`:''}
...
window.addEventListener('beforeinstallprompt', e => {
  ...
  const b = qs('#installBanner'); if (b) b.style.display = 'flex';
});
